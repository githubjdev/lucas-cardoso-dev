...
public class ProdutoController {
	...
	public ResponseEntity<Produto> buscarPorId(@PathVariable(name = "id") Long id) {
		...
		if (id <= 0) {
			throw new MsgApiException("Produto com cod: " + id + " não pode ser consultado.", HttpStatus.NOT_FOUND);
		}

		Optional<Produto> produto = produtoService.buscarPorId(id);
		
		if (!produto.isPresent()) {
			throw new MsgApiException("Produto com cod: " + id + " não existe.", HttpStatus.NOT_FOUND);
		}
